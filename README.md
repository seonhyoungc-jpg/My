...
│   ├── 07_video_maker.md          # 영상 콘텐츠
│   └── 08_analytics_tracker.md    # 검색 노출·통계 추적
├── guides/                   # 📚 규칙 문서
│   ├── style_guide.md             # 문체 가이드
│   ├── seo_guide.md               # 네이버 SEO 가이드
│   ├── qa_checklist.md            # QA 체크리스트(품질 게이트)
│   ├── prohibited_expressions.md  # 금지 표현
│   └── scheduling_guide.md        # 스케줄·콘텐츠 보드·예약발행
├── templates/                # 🧩 작성 양식
│   ├── blog_post_template.md
│   ├── keyword_map.md
│   └── content_board.csv          # Google Sheets 콘텐츠 보드
└── workflow/                 # 🔄 파이프라인 & 스케줄
    ├── pipeline.md
    ├── content_calendar.md        # 주 3일 발행 캘린더
    └── example_run.md
```
...
## 핵심 특징
- **역할 분리**: 8개 에이전트가 각자 산출물만 책임
- **품질 게이트**: QA(중간 테스트) 통과 전엔 발행 단계로 못 넘어감
- **네이버 최적화**: C-Rank/D.I.A. 대응 SEO 규칙 내장
- **정책 안전**: 금지 표현·저작권·과장광고 필터링
- **스케줄 관리**: 주 3일(월/수/금) 발행 캘린더 + Google Sheets/Notion 콘텐츠 보드
- **영상 콘텐츠**: 숏폼/클립 자동 기획·생성으로 체류시간 강화
- **성과 추적 루프**: 발행 후 검색 노출·조회수 추적 → 다음 기획에 피드백
